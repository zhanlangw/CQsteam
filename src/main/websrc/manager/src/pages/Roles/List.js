...
export default class Role extends PureComponent{
  ...
  handleSubmit = () => {
    const { dispatch,form:{validateFields},role:{ role_params } } = this.props;
    const self = this;
  ...
  handleReset = () => {
    const { dispatch,form:{resetFields},role:{ role_params } } = this.props;
    this.get_data({
      count:role_params.count,
      start:0
    });
  ...
  componentDidMount(){
    const { dispatch, role:{ role_params } } = this.props;
    this.get_data(role_params);
  }

  get_data = params => {
    const { dispatch } = this.props;
    dispatch({
      type:'role/fetch_role_list',
      payload:params
    })
  }
  ...
  handlePageChange = ({current}) => {
    const {role:{ role_params },dispatch} = this.props;
    const start = (current-1)*role_params.count;
    this.get_data({
      ...role_params,
      start:start
    });
  ...
  render(){
    const columns = [{
      ...
      render:val=>(<span style={{display: 'flex',justifyContent: 'space-around', color:'#005d7e'}}>
        <Tooltip title="详情"><Icon onClick={()=>router.push(`/roles/list/item/${val}`)} style={{cursor:'pointer'}} type="snippets" /></Tooltip> 
        <Tooltip title="修改"><Icon onClick={()=>router.push(`/roles/list/edit/${val}`)} style={{cursor:'pointer'}} type="form" /></Tooltip> 
        <Tooltip title="删除"><Icon onClick={()=>this.handleDel(val)} style={{cursor:'pointer'}} type="delete" /></Tooltip> 
      </span>)
    }];

    const { role:{role_list, role_params }, loading, form:{ getFieldDecorator } } = this.props;
    const { value, totalCount } = role_list;
    const { start, count, name, startTime, endTime, creator } = role_params;
    ...
    return (
      ...
      <Drawer
            ...
            <div className='header'>
               <Button onClick={()=>router.push(`/roles/list/add`)} type="primary" icon="plus">新增角色</Button>
