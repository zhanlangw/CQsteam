...
export default class Document extends PureComponent{
  ...
  componentDidMount(){
    const { dispatch, statistics:{ document_data_params:{ line1,line2 } } } = this.props;
    dispatch({
    ...
    dispatch({
      type:'statistics/fetch_document_data',
      payload:{
        ...line1
      },
      params:{
  ...
  categoryChange = (value,type,stage) => {
    const { dispatch,form:{ setFieldsValue }, statistics:{ document_areaOption, document_groupOption, document_data_params }  } = this.props;
    dispatch({
    ...
    dispatch({
      ...
      payload:{
        ...
        [`line${type}`]:{
          ...document_data_params[`line${type}`],
          categoryId:value,
        }
      },
      params:{
  ...
  loadCascaderData = (selectedOptions,index) => {
    let { dispatch, statistics:{ document_areaOption } } = this.props;
  ...
  cascaderChange = (value,index) => {
    const { dispatch, form:{ setFieldsValue }, statistics:{ document_data_params } } = this.props;
    dispatch({
      type:'statistics/changeDataParams',
      payload:{
        ...document_data_params,
        [`line${index}`]:{
          ...document_data_params[`line${index}`],
          areaId:value,
        }
      },params:{
        type:5
  ...
  groupChange = (value,index) => {
    const { dispatch, form:{ setFieldsValue, getFieldValue }, statistics:{ document_schoolOption, document_data_params } } = this.props;
  ...
  schoolChange = (value,index) => {
    const { dispatch, statistics:{ document_data_params } } = this.props;
    dispatch({
      type:'statistics/changeDataParams',
      payload:{
        ...document_data_params,
  ...
  handleSubmit = () => {
    const { dispatch,form:{validateFields}, statistics:{ document_data_params:{ line1, line2 } } } = this.props;
  ...
  render(){
    const { statistics:{ 
      document_data:{ line1,line2 }, 
      document_data_params:{ line1:line1_params,line2:line2_params }, 
      category_list,
      document_category_item:{ line1:line1_category,line2:line2_category },
      document_areaOption,
      document_groupOption,
      document_schoolOption
            ...
            <Spin spinning={loading}>  
                    ...
                    <FormItem
                        ...
                        {getFieldDecorator(`params[0][categoryId]`,{
                          initialValue:line1_params.categoryId
                            ...
                            <Select onChange={(val)=>this.categoryChange(val,1,5)} style={{width:'100%'}}>
                                {
                  ...
                  <Col span={6}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[0][areaId]`,{
                          ...
                          <Cascader  allowClear={false} placeholder='' onChange={(value)=>this.cascaderChange(value,1)} loadData={(selectedOptions)=>this.loadCascaderData(selectedOptions,1)} options={document_areaOption.line1} style={{width:'100%'}} />
                        )}
                    </FormItem>
                  </Col>
                  <Col span={3}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[0][group]`,{
                          ...
                          <Select onChange={(value)=>this.groupChange(value,1)}  style={{width:'100%'}}>
                            {
                              document_groupOption.line1.map(e=>{
                                if(e === 1){
                        ...
                        {getFieldDecorator(`params[0][schoolId]`,{
                          initialValue:line1_params.schoolId
                            ...
                            <Select onChange={(val)=>this.schoolChange(val,1)} style={{width:'100%'}}>
                              {
                                document_schoolOption.line1.map(e=>{
                    ...
                    <FormItem
                        ...
                        {getFieldDecorator(`params[1][categoryId]`,{
                          initialValue:line2_params.categoryId
                            ...
                            <Select onChange={(val)=>this.categoryChange(val,2,5)}  style={{width:'100%'}}>
                              {
                                category_list.map(e=><Select.Option key={e.id} value={e.id}><Tooltip title={e.name}>{e.name}</Tooltip></Select.Option>)
                              }
                            </Select>
                  ...
                  <Col span={6}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[1][areaId]`,{
                          ...
                          <Cascader  allowClear={false} placeholder='' onChange={(value)=>this.cascaderChange(value,2)} loadData={(selectedOptions)=>this.loadCascaderData(selectedOptions,2)} options={document_areaOption.line2} style={{width:'100%'}} />
                        )}
                    </FormItem>
                  </Col>
                  <Col span={3}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[1][group]`,{
                          ...
                          <Select onChange={(value)=>this.groupChange(value,2)}  style={{width:'100%'}}>
                            {
                              document_groupOption.line2.map(e=>{
                                if(e === 1){
                                  return <Option key={e} value={e}>小学</Option>
                                }else if(e === 2){
                                  return <Option key={e} value={e}>初中</Option>
                        ...
                        {getFieldDecorator(`params[1][schoolId]`,{
                          initialValue:line2_params.schoolId
                            ...
                            <Select onChange={(val)=>this.schoolChange(val,2)} style={{width:'100%'}}>
                              {
                                document_schoolOption.line2.map(e=>{
                                    return <Option key={e.id} value={e.id}>
                                              {e.name}
                                          </Option>
                                })
                ...
                <div style={{marginTop:24,width:600}}>
                  <Row gutter={16}>
                    ...
                    <Col span={3}>
                      <a href={`/api/stats/submit/export?${stringify(exprot_params)}`}><Button loading={loading} style={{marginTop:3}}>导出</Button></a>
                    </Col>
                  </Row>
                </div>
                <div>
