...
export default class Enter extends PureComponent{
  ...
  componentDidMount(){
    const { dispatch, statistics:{ enter_data_params:{ line1,line2 } } } = this.props;
    dispatch({
    ...
    dispatch({
      type:'statistics/fetch_enter_data',
      payload:{
        ...line1
      },
      params:{
  ...
  categoryChange = (value,type,stage) => {
    const { dispatch,form:{ setFieldsValue }, statistics:{ enter_areaOption, enter_groupOption, enter_data_params }  } = this.props;
    dispatch({
  ...
  loadCascaderData = (selectedOptions,index) => {
    let { dispatch, statistics:{ enter_areaOption } } = this.props;
  ...
  cascaderChange = (value,index) => {
    const { dispatch, form:{ setFieldsValue }, statistics:{ enter_data_params } } = this.props;
    dispatch({
      type:'statistics/changeDataParams',
      payload:{
        ...enter_data_params,
        [`line${index}`]:{
          ...enter_data_params[`line${index}`],
          areaId:value,
        }
      },params:{
        type:1
  ...
  groupChange = (value,index) => {
    const { dispatch, form:{ setFieldsValue, getFieldValue }, statistics:{ enter_schoolOption, enter_data_params } } = this.props;
  ...
  schoolChange = (value,index) => {
    const { dispatch, statistics:{ enter_data_params } } = this.props;
    dispatch({
      type:'statistics/changeDataParams',
      payload:{
        ...enter_data_params,
  ...
  handleSubmit = () => {
    const { dispatch,form:{validateFields}, statistics:{ enter_data_params:{ line1, line2 } } } = this.props;
  ...
  render(){
    const { statistics:{ 
      enter_data:{ line1,line2 }, 
      enter_data_params:{ line1:line1_params,line2:line2_params }, 
      category_list,
      enter_category_item:{ line1:line1_category,line2:line2_category },
      enter_areaOption,
      enter_groupOption,
      enter_schoolOption
            ...
            <Spin spinning={loading}>  
                    ...
                    <FormItem
                        ...
                        {getFieldDecorator(`params[0][categoryId]`,{
                          initialValue:line1_params.categoryId
                            ...
                            <Select onChange={(val)=>this.categoryChange(val,1,1)} style={{width:'100%'}}>
                                {
                  ...
                  <Col span={6}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[0][areaId]`,{
                          ...
                          <Cascader  allowClear={false} placeholder='' onChange={(value)=>this.cascaderChange(value,1)} loadData={(selectedOptions)=>this.loadCascaderData(selectedOptions,1)} options={enter_areaOption.line1} style={{width:'100%'}} />
                        )}
                    </FormItem>
                  </Col>
                  <Col span={3}>
                        ...
                        {getFieldDecorator(`params[0][schoolId]`,{
                          initialValue:line1_params.schoolId
                            ...
                            <Select onChange={(val)=>this.schoolChange(val,1)} style={{width:'100%'}}>
                              {
                                enter_schoolOption.line1.map(e=>{
                    ...
                    <FormItem
                        ...
                        {getFieldDecorator(`params[1][categoryId]`,{
                          initialValue:line2_params.categoryId
                            ...
                            <Select onChange={(val)=>this.categoryChange(val,2,1)}  style={{width:'100%'}}>
                              {
                                category_list.map(e=><Select.Option key={e.id} value={e.id}><Tooltip title={e.name}>{e.name}</Tooltip></Select.Option>)
                              }
                            </Select>
                  ...
                  <Col span={6}>
                    <FormItem
                        ...
                        {getFieldDecorator(`params[1][areaId]`,{
                          ...
                          <Cascader  allowClear={false} placeholder='' onChange={(value)=>this.cascaderChange(value,2)} loadData={(selectedOptions)=>this.loadCascaderData(selectedOptions,2)} options={enter_areaOption.line2} style={{width:'100%'}} />
                        )}
                    </FormItem>
                  </Col>
                  <Col span={3}>
                        ...
                        {getFieldDecorator(`params[1][schoolId]`,{
                          initialValue:line2_params.schoolId
                            ...
                            <Select onChange={(val)=>this.schoolChange(val,2)} style={{width:'100%'}}>
                              {
                                enter_schoolOption.line2.map(e=>{
                                    return <Option key={e.id} value={e.id}>
                                              {e.name}
                                          </Option>
                                })
                ...
                <div style={{marginTop:24,width:600}}>
                  <Row gutter={16}>
                    ...
                    <Col span={3}>
                      <a href={`/api/stats/sign_up/export?${stringify(exprot_params)}`}><Button loading={loading} style={{marginTop:3}}>导出</Button></a>
                    </Col>
                  </Row>
                </div>
                <div>
