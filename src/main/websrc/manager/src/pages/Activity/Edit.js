...
export default class Edit extends PureComponent{
  ...
  componentDidMount(){
    ...
    dispatch({
      type:'activity/fetch_activity_item',
      payload:{
       id:id
      },
    })
  ...
  onEndChange = (value) => {
    ...
    handleSubmit = () => {
        ...
        validateFields((err, values) => {
          if (!err) {
            dispatch({
                type:'activity/fetch_activity_upd',
                payload:{
                 ...values,
  ...
  render(){
    const { form:{ getFieldDecorator }, loading, activity:{ activity_item }, match:{params:{id}} } = this.props;
    const { startValue, endValue } = this.state;
    const item = activity_item[id] || {};
    const { title, startTime, endTime, stage, address  } = item;
    return (
      <Card bordered={false}>
            <Card>
                <FormItem
