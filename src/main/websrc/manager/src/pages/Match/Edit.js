...
export default class Add extends Component{
        ...
        dispatch({
          ...
          callback:(value)=>{
            dispatch({
              ...
              callback:(area)=>{
                this.setState({
                  checkedList:value.stages.map(e=>e.type),
                  checkAll:area.length === value.area.length,
                  indeterminate:area.length !== value.area.length,
                })
              }
            })
          }
    ...
    handleSubmit = () => {
        const { dispatch,form:{validateFields}, match:{params:{id}}, category:{ match_item } } = this.props;
        const item = match_item[id] || {};
  ...
  render(){
    const { form:{ getFieldDecorator }, loading, area, category:{ match_item }, match:{params:{id}}} = this.props;
    const { checkedList, checkAll, indeterminate } = this.state;
    const item = match_item[id] || {};
    const { area:item_area=[],
            creator, 
            updateFlag, 
            updateTime,
            telephoneType,
    ...
    const matchs = checkedList.sort().map((e,i)=>{
        ...
        const type = data.type || e; 
        disabled = status>type;
        const typeText = type===1?'初赛':type===2?'复赛':'决赛';
        return (
            <div key={e}>
                ...
                <FormItem
                    ...
                    {getFieldDecorator(`stages[${i}][endTime]`, {
                        ...
                        initialValue:data.endTime && moment(data.endTime) || undefined
                    })(
            ...
            <Card>
                <FormItem
                    ...
                    {getFieldDecorator('name', {
                        ...
                        <Input disabled={status!==1} placeholder="请填写类别名称"/>
                    )}
                </FormItem>
                <FormItem
                    {...formProps}
                    ...
                    {getFieldDecorator('abbreviation', {
                        ...
                        <Input disabled={status!==1} placeholder="请填写简称"/>
                    )}
                </FormItem>
                <Row gutter={10}>
                    <Col span={8}>
                    ...
                    <Col span={4}>
                        <FormItem
                            ...
                            {getFieldDecorator('minMember', {
                                ...
                                initialValue:minMember
                            })(
                                <InputNumber disabled={status!==1} onBlur={this.handleConfirmBlur} min={1}/>
                            )}
                        </FormItem>
                    </Col>
                    <Col span={4}>
                ...
                <FormItem
                    ...
                    {getFieldDecorator('group', {
                        ...
                        <Checkbox.Group disabled={status!==1} style={{ width: "100%", marginTop:9 }}>
                            <Row>
                ...
                <FormItem
                    ...
                    <div style={{
                            ...
                            <Row style={{marginLeft:-24}}>
                                <Col span={24}><Checkbox disabled={status!==1} checked={checkAll} indeterminate={indeterminate} onChange={this.checkAll} value='*'>全选</Checkbox></Col>
                            </Row>
                            {getFieldDecorator('areaIds', {
                                rules: [{ required: true, message: '省份配置不能为空' }],
                                initialValue:item_area.map(e=>e.id)
                            })(
                                <Checkbox.Group disabled={status!==1} onChange={this.areaIdsChange} style={{ width: "100%", marginTop:-6 }}>
                                    <Row>
                ...
                <FormItem
                    ...
                    {getFieldDecorator('telephoneType', {
                        ...
                        initialValue:telephoneType
                    })(
                        <Radio.Group disabled={status!==1} style={{ width: "100%"}}>
                ...
                <FormItem
                    ...
                    {getFieldDecorator('signUpTime', {
                        ...
                        initialValue:signUpTime && moment(signUpTime) || undefined
                    })(
                        <DatePicker disabled={status!==1} showTime style={{width:'100%'}} format="YYYY/MM/DD HH:mm:ss" placeholder="请选择报名开始时间"/>
                    )}
                </FormItem>
                <FormItem
                    {...formProps}
                    ...
                    {getFieldDecorator('match', {
                        ...
                        initialValue:stages.map(e=>e.type)
                    })(
                        <Checkbox.Group disabled={status!==1} onChange={this.onChange} style={{ width: "100%", marginTop:9 }}>
                            <Row>
                ...
                <div style={{textAlign:'center'}}>
                    <Button disabled={status>3} loading={loading} onClick={this.handleSubmit} type="primary">确定</Button>
